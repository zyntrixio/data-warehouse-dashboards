...
,calculate_status_times AS (
    SELECT
        ...
        ,COALESCE(LEAD(EVENT_DATE) OVER (PARTITION BY USER_ID, LOYALTY_PLAN ORDER BY EVENT_DATE), CURRENT_DATE()) - EVENT_DATE AS DAYS_IN_CURRENT_STATUS
        ,PREVIOUS_STATUS
        ,EVENT_DATE - LAG(EVENT_DATE) OVER (PARTITION BY USER_ID, LOYALTY_PLAN ORDER BY EVENT_DATE) AS DAYS_IN_PREVIOUS_STATUS
    FROM
