...
,select_status_changes AS ( -- Just get events for which there is a status change. Calculate Registration date
    SELECT
        ...
        ,BRAND
        ,STATUS
        ,LAG(STATUS) OVER (PARTITION BY USER_ID, LOYALTY_PLAN, LOYALTY_PLAN_COMPANY ORDER BY EVENT_DATE_TIME) AS PREVIOUS_STATUS
...
,calculate_status_times AS (
    SELECT
        ...
        ,PREVIOUS_STATUS
        ,STATUS_FROM_DATE - LAG(STATUS_FROM_DATE) OVER (PARTITION BY USER_ID, LOYALTY_PLAN, LOYALTY_PLAN_COMPANY ORDER BY STATUS_FROM_DATE) AS DAYS_IN_PREVIOUS_STATUS
