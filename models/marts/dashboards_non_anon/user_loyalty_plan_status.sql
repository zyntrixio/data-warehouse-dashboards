...
,day_ends AS ( -- just get events that finish the day
    SELECT
        *
        ,MAX(EVENT_DATE_TIME) OVER (PARTITION BY USER_ID, LOYALTY_PLAN, STATUS_FROM_DATE) AS LAST_EVENT_DAILY
...
,calculate_status_times AS (
    SELECT
        USER_ID
        ,STATUS_FROM_DATE
        ,LEAD(STATUS_FROM_DATE) OVER (PARTITION BY USER_ID, LOYALTY_PLAN ORDER BY STATUS_FROM_DATE) AS STATUS_TO_DATE
        ,LOYALTY_PLAN
        ,BRAND
        ,STATUS AS CURRENT_STATUS
        ,COALESCE(LEAD(STATUS_FROM_DATE) OVER (PARTITION BY USER_ID, LOYALTY_PLAN ORDER BY STATUS_FROM_DATE), CURRENT_DATE()) - STATUS_FROM_DATE AS DAYS_IN_CURRENT_STATUS
        ,PREVIOUS_STATUS
        ,STATUS_FROM_DATE - LAG(STATUS_FROM_DATE) OVER (PARTITION BY USER_ID, LOYALTY_PLAN ORDER BY STATUS_FROM_DATE) AS DAYS_IN_PREVIOUS_STATUS
    FROM
