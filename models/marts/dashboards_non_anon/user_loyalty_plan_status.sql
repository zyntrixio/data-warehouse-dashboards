WITH user_events AS (
    ...
    AND LOYALTY_PLAN_COMPANY IS NOT NULL
    ...
    SELECT
        *
        ,EVENT_DATE_TIME::DATE - MAX(CASE WHEN EVENT = 'TRANSACT' THEN EVENT_DATE_TIME END)
            OVER (PARTITION BY USER_ID, LOYALTY_PLAN, LOYALTY_PLAN_COMPANY ORDER BY EVENT_DATE_TIME ROWS BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW)::DATE
            AS DAYS_SINCE_LAST_TRANSACT_EVENT
        ,MIN(CASE WHEN EVENT = 'REFRESH' THEN EVENT_DATE_TIME END)
            OVER (PARTITION BY USER_ID, LOYALTY_PLAN, LOYALTY_PLAN_COMPANY ORDER BY EVENT_DATE_TIME ROWS BETWEEN 1 FOLLOWING AND UNBOUNDED FOLLOWING)::DATE - EVENT_DATE_TIME::DATE
...
,day_ends AS ( -- just get events that finish the day
    SELECT
        *
        ,MAX(EVENT_DATE_TIME) OVER (PARTITION BY USER_ID, LOYALTY_PLAN, LOYALTY_PLAN_COMPANY, STATUS_FROM_DATE) AS LAST_EVENT_DAILY
...
,select_status_changes AS ( -- Just get events for which there is a status change. Calculate Registration & Deletion dates date
    SELECT
        ...
        ,STATUS
        ,CASE
            WHEN STATUS != 'REGISTRATION' THEN LAG(STATUS) OVER (PARTITION BY USER_ID, LOYALTY_PLAN, LOYALTY_PLAN_COMPANY ORDER BY EVENT_DATE_TIME) 
            END AS PREVIOUS_STATUS
        ,STATUS_FROM_DATE - MAX(CASE WHEN EVENT = 'LC_REGISTER' THEN EVENT_DATE_TIME END)
            OVER (PARTITION BY USER_ID, LOYALTY_PLAN, LOYALTY_PLAN_COMPANY ORDER BY EVENT_DATE_TIME ROWS BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW)::DATE AS DAYS_SINCE_REGISTRATION
        ,STATUS_FROM_DATE - MAX(CASE WHEN EVENT = 'LC_REMOVE' THEN EVENT_DATE_TIME END)
            OVER (PARTITION BY USER_ID, LOYALTY_PLAN, LOYALTY_PLAN_COMPANY ORDER BY EVENT_DATE_TIME ROWS BETWEEN UNBOUNDED PRECEDING AND 1 PRECEDING)::DATE AS DAYS_SINCE_DELETION
    FROM
        day_ends
    QUALIFY
        ...
        AND
        (DAYS_SINCE_REGISTRATION < DAYS_SINCE_DELETION)
)
...
,calculate_status_times AS (
    SELECT
        USER_ID
        ,STATUS_FROM_DATE
        ,LEAD(STATUS_FROM_DATE) OVER (PARTITION BY USER_ID, LOYALTY_PLAN, LOYALTY_PLAN_COMPANY ORDER BY STATUS_FROM_DATE) AS STATUS_TO_DATE
        ,LOYALTY_PLAN
        ,LOYALTY_PLAN_COMPANY
        ,BRAND
        ,STATUS AS CURRENT_STATUS
        ,COALESCE(LEAD(STATUS_FROM_DATE) OVER (PARTITION BY USER_ID, LOYALTY_PLAN, LOYALTY_PLAN_COMPANY ORDER BY STATUS_FROM_DATE), CURRENT_DATE()) - STATUS_FROM_DATE AS DAYS_IN_CURRENT_STATUS
        ,PREVIOUS_STATUS
        ,CASE
            WHEN PREVIOUS_STATUS IS NOT NULL THEN STATUS_FROM_DATE - LAG(STATUS_FROM_DATE) OVER (PARTITION BY USER_ID, LOYALTY_PLAN, LOYALTY_PLAN_COMPANY ORDER BY STATUS_FROM_DATE)
