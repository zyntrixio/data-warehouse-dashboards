...
,generate_refresh_events AS (
    SELECT
        *
        ,DATEDIFF(D,CREATED_DT,DELETE_DT) AS DAY_DIFF
        ,CASE WHEN UNIFORM(0,10, RANDOM()) - 3 < 0 THEN 0 ELSE UNIFORM(0,7, RANDOM()) END AS NUM_REFRESHES
        ,CAST((DAY_DIFF/(NUM_REFRESHES+1)) -1 AS INT) AS DAY_DIFF_1
...
,union_all_refresh_dts AS (
    SELECT USER_ID, REFRESH_DT_1 AS REFRESH_DT
    FROM generate_refresh_events
    WHERE REFRESH_DT_1 IS NOT NULL
    UNION ALL
    SELECT USER_ID, REFRESH_DT_2 AS REFRESH_DT
    FROM generate_refresh_events
    WHERE REFRESH_DT_2 IS NOT NULL
    UNION ALL
    SELECT USER_ID, REFRESH_DT_3 AS REFRESH_DT
    FROM generate_refresh_events
    WHERE REFRESH_DT_3 IS NOT NULL
    UNION ALL
    SELECT USER_ID, REFRESH_DT_4 AS REFRESH_DT
    FROM generate_refresh_events
    WHERE REFRESH_DT_4 IS NOT NULL
    UNION ALL
    SELECT USER_ID, REFRESH_DT_5 AS REFRESH_DT
    FROM generate_refresh_events
